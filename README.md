...
サイト構成は「オープニングページ（写真スライドショー or 動画）→ トップページ（縦長写真＋あいさつ＋8個の正方形ボタン）→ 各詳細ページ（プロフィール／生い立ち／二人の道／お座席／お食事）」の3層です。詳細ページでは右上のハンバーガーメニューから他のページへ移動できます（トップページにはハンバーガーは表示されません）。

## 内容の差し替え方

`js/data.js` を編集してください。オープニングの写真・動画（`OPENING`）、ゲスト名・続柄・テーブル、挨拶文、お食事メニュー、プロフィール、ナビボタンのラベル（`NAV_ITEMS`）をすべてここで管理しています。HTML/CSSを触る必要はありません。
